<?php
namespace Src\Validation; class Validator {
    private array $data; private array $rules; private array $errors = [];
    private function __construct($data, $rules) {
        $this->data = $data; $this->rules = $rules;}
        ...
        foreach ($this->rules as $f => $r) {
            ...
            foreach (explode('|', $r) as $rule) {
                ...
                elseif (str_starts_with($rule, 'max:') &&
                    ...
                    $this->errors[$f][] = $rule;
                elseif ($rule == 'email' &&
                    $val != null && !filter_var($val, FILTER_VALIDATE_EMAIL))
                    $this->errors[$f][] = 'email';
    ...
    public static function sanitize(array $in) { foreach ($in as $k => $v) {
            if (is_string($v)) { $in[$k] = trim($v);}}
                return $in;}
}
